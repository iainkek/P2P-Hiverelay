...
| `escrow/contracts/PokerEscrow.sol` | 08 | USD₮ state-channel escrow — **deposit / settle-net / withdraw** (cooperative + committee-dispute; no deposit-refund escape; multi-hand settle; cheat/stall/coop paths; opt-in funding lock), **reentrancy-hardened (CEI + guard)**, real-USDT-safe transfers; threat model in [SECURITY.md](escrow/SECURITY.md) | 31 |
...
- **External audit** of `PokerEscrow.sol` + the attestation signature scheme
  ...
  threat model, invariants, and the one remaining design-choice finding (F-3
  liveness backstop) are pre-staged in
  [escrow/SECURITY.md](escrow/SECURITY.md) to make that audit faster; the token
  integration (F-1/F-2), event transparency (F-5), and deposit-lock (F-4) are
